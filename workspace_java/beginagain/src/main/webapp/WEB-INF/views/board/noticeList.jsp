...
<c:set var="pageSubTitle" value="공지사항 조회 / 수정 / 삭제" />
								...
								<c:choose>
									<c:when test="${not empty noticeList}">
										<c:forEach var="n" items="${noticeList}">
											<tr>
												<td><input type="checkbox" name="noticeId"
													value="${n.noticeId}"></td>
												<td>${n.noticeId}</td>
												<td>${n.title}</td>
												<td>${n.writerEmpName}</td>
												<td>${n.status}</td>
												<td>${n.viewCount}</td>
												<td><a class="btn"
													href="${pageContext.request.contextPath}/notice/detail?noticeId=${n.noticeId}">상세</a></td>
											</tr>
										</c:forEach>
									</c:when>
									<c:otherwise>
										<tr>
											<td colspan="7">조회된 공지사항이 없습니다.</td>
										</tr>
									</c:otherwise>
								</c:choose>
							</tbody>
		...
		document.getElementById("checkAll").addEventListener(
				...
				function() {
					const checks = document
							.querySelectorAll("input[name='noticeId']");
...
<div class="modal" id="registerModal">
    <div class="modal-box">
        <div class="modal-header">
            <h3 class="modal-title">공지사항 등록</h3>
...
<div class="form-row"><label>상태</label><input type="text" name="status" value="게시"></div>
