...
public class MemberDAO {
    ...
    public List<MemberDTO> selectMemberList(Connection conn) {
        ...
        String sql = ""
                ...
                + "       STATUS, ROLE_NAME, USE_YN, TEMP_PWD_YN, REMARK, CREATED_AT, UPDATED_AT "
                + "FROM EMP "
                + "WHERE USE_YN = 'Y' "
...
public int insertMember(Connection conn, MemberDTO dto) {
    PreparedStatement ps = null;
    int result = 0;
    String sql = "INSERT INTO EMP (EMP_ID, EMP_NO, EMP_NAME, PASSWORD_HASH, DEPT_CODE, POSITION_NAME, EMAIL, PHONE, STATUS, ROLE_NAME, USE_YN, TEMP_PWD_YN, REMARK, CREATED_AT, UPDATED_AT) VALUES (SEQ_EMP.NEXTVAL, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'Y', 'Y', ?, SYSDATE, SYSDATE)";
    try {
        ps = conn.prepareStatement(sql);
