...
public class test {
	...
	public static void main(String[] args) {
		...
		boolean run =true;
		int balance =0;
		String balp0 = " ";					// 가장 최근 입출금 내역
		String balp1 = " ";					// 과거 입출금 내역
		String balp2 = " ";
		/*
		String balp%n = " ";				// 안 됨	
		*/
		while(run) {
			...
			if (ch ==1) { 
				...
				if(m<0) { 								// 예금액이 음수일 경우
					...
					balp2 = balp1;						// 과거 기록 저장	
					balp1 = balp0;
					balance += m;						// 예금액을 잔고에 더함
					balp0 = "예금 "+ m +" 잔액 "+ balance; // 현재 기록
				}
			} else if(ch ==2) {
				...
				if (m<0) {								// 출금액이 음수일 경우
					...
					balp0 = "출금 "+m +" 잔액 "+ balance; //현재기록					
				}
			} else if(ch==3) {
				System.out.println("잔고> "+balance);	// 잔고 보여줌
				System.out.println("입출금 내역>"); 	// 과거 기록 보여줌
				System.out.println("3. "+balp0);
				System.out.println("2. "+balp1);
				System.out.println("1. "+balp2);
